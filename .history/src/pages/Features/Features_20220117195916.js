...
const Features = () => {
  ...
  useEffect(() => { if (checkouts.length <= 3) {
    const c= parseInt(fee) * checkouts.length
    console.log(c)
    setData(data=>({ ...data, transportFee:24 }));
    console.log("< 3");
  ...
  useEffect(() => { setData({ ...data, details: checkouts.map((c) => c.id) })},[checkouts])
  useEffect(() => {
    setAllTotal(
