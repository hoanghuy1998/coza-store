...
const Features = () => {
  ...
  useEffect(() => {
    ...
    if (checkouts.length <= 3) {
      setData({ ...data, transportFee: fee * checkouts.length });
      console.log("< 3");
    } else if (checkouts.length > 3 && checkouts.length <= 6) {
      console.log("> 3 <6");

      setData({ ...data, transportFee: fee });
    } else {
      console.log("do day");
      setData({ ...data, transportFee: 0 });
    }
    const x = checkouts.map((c) => c.id);
    console.log("x", x);
    setData({ ...data, details: x });
  }, [checkouts]);
  useEffect(() => {
    setAllTotal(
      (allTotal) => (allTotal = sale + cartTotal + data.transportFee)
    );
  }, [sale, cartTotal, transport]);
  ...
  const handleChangeData = (e, id, quantity) => {
    if (parseInt(e.target.value) === 0) {
      ...
      mycartService.update(id, data).then((res) => {
        ...
        toast.info(`đã cập nhật lại số lượng  ${data.name} thành công `);
        loadData(res.data);
      });
      loadData();
    }
