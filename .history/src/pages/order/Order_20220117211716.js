    ...
    productSolded.getCodeOrder(list.codeOrder).then((res) => {
      if (res.data.errorCode === 0) {
        ...
        dispatch({
          ...
          dataCheckOut: {
            transportFee:listcheckout.transportFee,
            coupon: listcheckout.coupon,
            x
          }
        });
        // navigate(`/order/${id}`);
                    ...
                    {listcheckout.map((list, index) => (
                      <tr className="text-center" key={index}>
                        ...
                        <td
                          ...
                          width="20%"
                        >
                          {list.status === 0 ? "New" :list.status === 1 ?"đã xác nhận":list.status === 2 ? "đang vận chuyển":'giao thành công'}
                        </td>
